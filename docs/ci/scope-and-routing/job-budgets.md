...
has 524,288 characters for every matrix together. Compact groups and full-plan
Node rows list each test file explicitly. The manifest packs the shard runner's
group fields with gzip+base64 (`groups_gzip_base64`) when the target contains the
codec; an ungrouped file-list row becomes one equivalent group without repeating
its raw file list. Job-level environment and scheduling fields remain unchanged.
Historical targets without that capability retain their raw single-row fields
or projected legacy `groups` JSON. Workflow tests keep compact PR, full manual,
and exact-head PR fallback output under half of the cap. The smaller
