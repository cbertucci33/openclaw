...
- `gateway.roles.definitions.<role>.accessPolicyPlugin`: optional exact plugin ID whose registered Gateway access policy must authorize people governed by this role. For example, a default visitor role can require `"visitor-access"`, while an independently assigned staff role omits the field. The Gateway denies this role's access when the plugin is missing or disabled, its manifest is missing or malformed, loading fails, or the policy supplies no current authority. The reference remains valid configuration while the plugin is unavailable, so independent staff and the Gateway owner can repair it. Omitting the field adds no plugin dependency. Changes to this field require a Gateway restart.
- `gateway.roles.definitions.<name>.modelPolicy`: optional model ceiling for a role's requests, queued work, and children. With `{}`, permitted models follow the designated source agent's primary and fallbacks. `sourceAgent` chooses that agent; `allow` replaces membership and `deny` excludes resolved model references, aliases, or trailing family patterns such as `provider/restricted-*`. An empty `allow` denies all models. With config reload enabled, edits confined to existing roles' model policies apply when committed without restarting the Gateway: requests using removed models are canceled while permitted models and unrelated work continue. Other role settings, role additions or removals, and default-role changes continue to require a restart, including when combined with a model-policy edit. Existing manual model rules still apply; omitted role model policy and System access are unchanged. See [Named operator roles](/gateway/operator-scopes#named-operator-roles).
